...
export class PagesPage extends Component {
  constructor(props) {
    ...
    this.handleDeleteAlert = this
      .handleDeleteAlert
      .bind(this);
    this.handleDelete = this
      .handleDelete
      .bind(this);
    this.refreshPages = this
      .refreshPages
      .bind(this);
  }
  ...
  refreshPages() {
    fetch("http://localhost:3001/api/pages")
      .then(res => res.json())
      .then(res => this.setState({pages: res, isLoading: false, showConfirm: false}))
      .catch(function (error) {
        // Catch errors here
      });
  }
            ...
            .map(page => {
              return (
                <li className="pages-list__item" key={page.pageId}>
                  ...
                  <p
                    className={`pages-list__item__status pages-list__item__status--${page.status}`}>
                    {page.status}
                  </p>
                </li>
              );
