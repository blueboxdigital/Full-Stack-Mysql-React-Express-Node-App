...
module.exports = {
  ...
  async create(req, res) {
    ...
    try {
      ...
      return res.cookie("auth", data.authToken.token, { maxAge: 864000000 }).json(data);
    } catch (err) {
  ...
  async login(req, res) {
    ...
    try {
      ...
      return (
        res.cookie("auth", user.authToken.token, { maxAge: 864000000 }),
        res.send("Check your cookies. One should be in there now")
      );
      //return res.json(user.authToken.token);
    } catch (err) {
      return res.status(400).send(`something went wrong, here is the error: ${err}`);
    }
  },
